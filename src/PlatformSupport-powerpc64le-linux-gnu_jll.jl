module PlatformSupport-powerpc64le-linux-gnu_jll
...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import PlatformSupport-powerpc64le-linux-gnu_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("b2c223ce-fe75-5e40-a76c-3cc3009db2af"))
platforms = [Pkg.Artifacts.unpack_platform(e, "PlatformSupport-powerpc64le-linux-gnu", artifacts_toml) for e in artifacts["PlatformSupport-powerpc64le-linux-gnu"]]
...
if best_platform === nothing
    @debug("Unable to load PlatformSupport-powerpc64le-linux-gnu; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module PlatformSupport-powerpc64le-linux-gnu_jll
